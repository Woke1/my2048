...
var musicPlayer = true;

$(document).ready(function(){
	var MusicControlObj = $('#music_control');
	var MusicTxtChange = MusicControlObj.find('span:eq(0)');
	MusicControl(MusicControlObj,MusicTxtChange);
...
//游戏音效
function musicPlay(){
	$('#music_1')[0].volume = 0.4;
	$('#music_1')[0].currentTime = 0;
	$('#music_1')[0].play();	
}
//音乐控制
function MusicControl(controlObj,musicChange){
	var aud = $('audio');
	var audSrc = [];
	aud.each(function(){
		audSrc.push($(this).attr('src'));
	});
	var musicPlay = false;
	controlObj.click(function(){
		if(musicPlay){
			aud.each(function(index){
				$(this).attr('src',audSrc[index]);
			});
			musicChange.html('关闭');
			musicPlay = false;
			return true;
